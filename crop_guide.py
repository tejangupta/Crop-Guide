...
class Ui_MainWindow(object):
    ...
    def submit(self):
        # dict 'res' had key as crop name and value has Pesticides
        res = {'Sugarcane': 'insecticides and fungi- cides',
        ...
        all_state = cur.execute('SELECT * FROM crop WHERE northern IS NOT NULL')

        # Fetch the output
        all_state = cur.fetchall()

        all_state = list(all_state[0][5:-1])  # Remove null values by converting into a list and using slice operator
        state = self.comboBox.currentText()  # Get text from the user(input) in case of combobox
        season = self.comboBox_2.currentText()
        acres = int(self.line_acres.text())
        budget = int(self.line_budget.text())  # Get text from the user(input) in case of line_edit

        ns = state in all_state[0]  # in operator return bool values

        if ns:
            if budget >= 50000 and acres >= 10:
                # fetch data from nh column to the w.r.t season
                cur.execute("SELECT nh FROM crop WHERE season=?", [season])
                crop_name = cur.fetchall()  # fetch the output
                crop_name = str(crop_name[0])[2:-3]

                self.label_image.setPixmap(QtGui.QPixmap("img//" + crop_name + ".png"))  # add image
                result = "Pesticides used are " + res[crop_name]  # "+" used to concat
                self.text_info.setText(result)  # add text to the line_edit
            else:
                cur.execute("SELECT nl FROM crop WHERE season=?", [season])
                crop_name = cur.fetchall()
                crop_name = str(crop_name[0])[2:-3]

                self.label_image.setPixmap(QtGui.QPixmap("img//" + crop_name + ".png"))
                self.text_info.setText(crop_name)
                result = "Pesticides used are " + res[crop_name]
                self.text_info.setText(result)
        else:
            if budget >= 50000 and acres >= 10:
                cur.execute("SELECT sh FROM crop WHERE season=?", [season])
                crop_name = cur.fetchall()
                crop_name = str(crop_name[0])[2:-3]
                ...
                cur.execute("SELECT sl FROM crop WHERE season=?", [season])
                crop_name = cur.fetchall()
                crop_name = str(crop_name[0])[2:-3]
